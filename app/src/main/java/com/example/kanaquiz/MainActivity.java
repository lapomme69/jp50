...
public class MainActivity extends Activity {
    ...
    private static final int MIC_REQUEST = 1701;
    ...
    private void setupRecognizer() {
        ...
        recognizer.setRecognitionListener(new RecognitionListener() {
            ...
            @Override public void onError(int e) {
                String msg = e == SpeechRecognizer.ERROR_INSUFFICIENT_PERMISSIONS
                        ? "마이크 권한이 필요합니다."
                        : e == SpeechRecognizer.ERROR_NO_MATCH
                        ? "음성을 듣지 못했습니다. 다시 시작해 주세요."
                        : "음성인식 오류입니다. 다시 시작해 주세요.";
                sendError(msg);
            }
        });
    }
    ...
    private void startListening() {
        try {
            ...
            if (recognizer == null) {
                sendError("이 휴대폰에서 음성인식을 사용할 수 없습니다.");
                return;
            }
            recognizer.cancel();
            ...
            sendError("마이크를 시작할 수 없습니다. 마이크 권한을 확인해 주세요.");
        }
    }
    ...
    private void speakJapanese(String text) {
        ...
        try {
            ...
            tts.setSpeechRate(0.8f);
            tts.speak(text, TextToSpeech.QUEUE_FLUSH, null, "kana-v17");
        } catch (Throwable ignored) {}
    }
    ...
    private void sendResult(String s) {
        main.post(() -> { try { webView.evaluateJavascript(
            "window.onNativeSpeechResult("+q(s)+")", null); } catch(Throwable ignored){} });
    }
    private void sendError(String s) {
        main.post(() -> { try { webView.evaluateJavascript(
            "window.onNativeSpeechError("+q(s)+")", null); } catch(Throwable ignored){} });
    }
